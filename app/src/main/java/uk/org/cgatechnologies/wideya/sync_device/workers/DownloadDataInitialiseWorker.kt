...
private const val TAG = "DownloadDataInitialiseWorker"

class DownloadDataInitialiseWorker(appContext: Context, workerParams: WorkerParameters) :
    CoroutineWorker(appContext, workerParams) {

    override suspend fun doWork(): Result {

        Utils.insertLog(applicationContext, "🔄  Initial sync started")
        ...
        do {
            ...
            Log.d(TAG, "Check Sync Download Data Initialise Response: $recordsCounter")
...
//                    Utils.insertLog(applicationContext, "$recordsCounter records inserted")
        ...
        return if (attempts == attemptsLimit || errors == errorsLimit) {
            ...
            Utils.getEncSharedPrefs(applicationContext).edit()
                .putBoolean(Constants.PREF_INITIAL_LOAD_REQUIRED_BOOL, false).apply()
            Result.success()
        }
    }
}